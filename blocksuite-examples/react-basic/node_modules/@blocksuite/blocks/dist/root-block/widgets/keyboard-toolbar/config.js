import { getInlineEditorByModel, insertContent, REFERENCE_NODE, } from '@blocksuite/affine-components/rich-text';
...
const textToolActionItems = [
    {
        name: 'Text',
        icon: TextIcon(),
        showWhen: ({ std }) => std.doc.schema.flavourSchemaMap.has('affine:paragraph'),
        action: ({ std }) => {
    ...
    },
    {
        name: 'Divider',
        icon: DividerIcon(),
        showWhen: ({ std }) => std.doc.schema.flavourSchemaMap.has('affine:divider'),
        action: ({ std }) => {
            std.command.exec('updateBlockType', {
    ...
    items: [
        {
            name: 'NewPage',
            icon: NewPageIcon(),
            showWhen: ({ std }) => std.doc.schema.flavourSchemaMap.has('affine:embed-linked-doc'),
            action: ({ std }) => {
                std.command
                    .chain()
                    .getSelectedModels()
    ...
    items: [
        {
            name: 'Image',
            icon: ImageIcon(),
            showWhen: ({ std }) => std.doc.schema.flavourSchemaMap.has('affine:image'),
            action: ({ std }) => {
                std.command
                    .chain()
                    .getSelectedModels()
        ...
        },
        {
            name: 'Link',
            icon: LinkIcon(),
            showWhen: ({ std }) => std.doc.schema.flavourSchemaMap.has('affine:bookmark'),
            action: async ({ std }) => {
                ...
                await toggleEmbedCardCreateModal(std.host, 'Links', 'The added link will be displayed as a card view.', { mode: 'page', parentModel, index });
        ...
        },
        {
            name: 'Attachment',
            icon: AttachmentIcon(),
            showWhen: ({ std }) => std.doc.schema.flavourSchemaMap.has('affine:attachment'),
            action: async ({ std }) => {
                const { selectedModels } = std.command.exec('getSelectedModels');
                const model = selectedModels?.[0];
                if (!model)
        ...
        },
        {
            ...
            showWhen: ({ std }) => std.doc.schema.flavourSchemaMap.has('affine:embed-youtube'),
            action: async ({ std }) => {
                const { selectedModels } = std.command.exec('getSelectedModels');
                const model = selectedModels?.[0];
                if (!model)
                ...
                if (!parentModel)
                    return;
                const index = parentModel.children.indexOf(model) + 1;
                await toggleEmbedCardCreateModal(std.host, 'YouTube', 'The added YouTube video link will be displayed as an embed view.', { mode: 'page', parentModel, index });
                if (model.text?.length === 0) {
                    std.doc.deleteBlock(model);
                }
            },
        },
        {
            ...
            showWhen: ({ std }) => std.doc.schema.flavourSchemaMap.has('affine:embed-github'),
            action: async ({ std }) => {
                const { selectedModels } = std.command.exec('getSelectedModels');
                const model = selectedModels?.[0];
                if (!model)
                ...
                if (!parentModel)
                    return;
                const index = parentModel.children.indexOf(model) + 1;
                await toggleEmbedCardCreateModal(std.host, 'GitHub', 'The added GitHub issue or pull request link will be displayed as a card view.', { mode: 'page', parentModel, index });
                if (model.text?.length === 0) {
                    std.doc.deleteBlock(model);
                }
            },
        },
        {
            ...
            showWhen: ({ std }) => std.doc.schema.flavourSchemaMap.has('affine:embed-figma'),
            action: async ({ std }) => {
                const { selectedModels } = std.command.exec('getSelectedModels');
                const model = selectedModels?.[0];
                if (!model)
                ...
                const index = parentModel.children.indexOf(model) + 1;
                await toggleEmbedCardCreateModal(std.host, 'Figma', 'The added Figma link will be displayed as an embed view.', { mode: 'page', parentModel, index });
                if (model.text?.length === 0) {
                    std.doc.deleteBlock(model);
                }
            },
        },
        {
            ...
            showWhen: ({ std }) => std.doc.schema.flavourSchemaMap.has('affine:embed-loom'),
            action: async ({ std }) => {
                const { selectedModels } = std.command.exec('getSelectedModels');
                const model = selectedModels?.[0];
                if (!model)
                ...
                if (!parentModel)
                    return;
                const index = parentModel.children.indexOf(model) + 1;
                await toggleEmbedCardCreateModal(std.host, 'Loom', 'The added Loom video link will be displayed as an embed view.', { mode: 'page', parentModel, index });
                if (model.text?.length === 0) {
                    std.doc.deleteBlock(model);
                }
            },
        },
        {
            name: 'Equation',
            icon: TeXIcon(),
            showWhen: ({ std }) => std.doc.schema.flavourSchemaMap.has('affine:latex'),
            action: ({ std }) => {
                std.command
                    .chain()
                    .getSelectedModels()
    ...
    items: [
        {
            ...
            showWhen: ({ std }) => std.doc.schema.flavourSchemaMap.has('affine:database'),
            action: ({ std }) => {
                std.command
                    .chain()
                    .getSelectedModels()
