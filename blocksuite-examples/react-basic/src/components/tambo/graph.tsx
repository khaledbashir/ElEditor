...
export const Graph = React.forwardRef<HTMLDivElement, GraphProps>(
  (
    { className, variant, size, data, title, showLegend = true, ...props }: GraphProps,
    ...
    try {
      ...
      const maxDataPoints = Math.min(
        data.labels.length,
        Math.min(...validDatasets.map((d: { data: number[] }) => d.data.length)),
      );
      ...
      const chartData = data.labels
        ...
        .map((label: string, index: number) => ({
          name: label,
          ...Object.fromEntries(
            validDatasets.map((dataset: { label: string; data: number[] }) => [
              dataset.label,
          ...
          case "bar":
            return (
              <RechartsCore.BarChart data={chartData}>
                ...
                {validDatasets.map((dataset: { label: string; color?: string }, index: number) => (
                  <RechartsCore.Bar
                    key={dataset.label}
